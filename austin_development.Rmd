...
auMSA <- st_read("/Users/luyiiwong/Documents/GitHub/LandUseModeling_HW5/AustinData/AustinMSA.geojson") %>%
  st_transform(crs = 2278)

lc_2011 = raster("/Users/luyiiwong/Documents/GitHub/LandUseModeling_HW5/AustinData/AU_lc_2011_Resample.tif")
lc_2021 = raster("/Users/luyiiwong/Documents/GitHub/LandUseModeling_HW5/AustinData/AU_lc_2021_Resample.tif")
...
fishnetPopulation21 <-
  st_interpolate_aw(njpop_2021["pop_2021"],auMSA_fishnet, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID") %>%
  left_join(auMSA_fishnet %>%
              mutate(fishnetID = as.character(fishnetID)),
...
highwayPoints_fishnet <- 
  aggregate(highwayPoints, auMSA_fishnet, mean) %>%
...
studyAreaCounties <- 
  ...
  st_transform(st_crs(auMSA)) %>%
  dplyr::select(NAME) %>%
  .[st_buffer(auMSA,-1000), , op=st_intersects] 
