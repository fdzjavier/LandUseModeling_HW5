...
```{r}
NJcounties <- 
  st_read("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/nj_counties_msa.geojson") 

lc_2011 <- raster("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/lc_nj_msa_2011.tif")

lc_2021 = raster("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/nlcd_2021_landcover.tif")
```


```{r conduct manual land cover change from 2011-2021}
reclassMatrix <- 
  matrix(c(
    0,12,0,
    12,24,1,
...
Resample rasters under the same resolution
```{r}
library(raster)

# Assuming e1 and e2 are your raster objects
developed_2021_resampled <- resample(developed_2021, developed_2011, method = "bilinear")

# Now both e1 and e2_resampled have the same resolution
compareRaster(developed_2011, developed_2021_resampled, extent = FALSE, rowcol = FALSE, crs = TRUE)
...
NJcounties <- 
  st_read("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/nj_counties_msa.geojson") 
lc_change = raster("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/nlcd_nj_msa.tif")
```
...
ggplot() +
  ...
  geom_raster(data=rast(lc_change) %>% na.omit %>% filter(value > 0), 
              aes(x,y,fill=as.factor(value))) +
  scale_fill_viridis(direction = -1, discrete=TRUE, name ="Land Cover/nChange") +
...
lc_change2[lc_change2 < 3] <- NA #map not reclassifying
...
ggplot() +
  ...
  geom_raster(data=rast(lc_change2) %>% na.omit, 
              aes(x,y,fill=as.factor(value))) +
  scale_fill_viridis(discrete=T, name ="Land Cover/nChange") + 
  labs(title="Development Land Use Change") +
  mapTheme
```
...
```{r, warning = FALSE, message = FALSE}
NJcounties_fishnet <- 
  st_make_grid(NJcounties, 2000) %>%
  st_sf()

NJcounties_fishnet <-
  NJcounties_fishnet[NJcounties,]
```
...
```{r, warning = FALSE, message= FALSE}
ggplot() +
  geom_sf(data=NJcounties_fishnet) +
...
changePoints <-
  ...
  as.data.frame() %>%
  st_as_sf(coords = c("x", "y"), crs = st_crs(NJcounties_fishnet))

fishnet <- 
  aggregate(changePoints, NJcounties_fishnet, sum) %>%
  mutate(lc_change = ifelse(is.na(lc_change),0,3),
...
geojson_file <- "C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/lc_fishnet.geojson"

# Export to GeoJSON
st_write(fishnet, geojson_file)

njgeojson_file <- "C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/nj_fishnet.geojson"

# Export to GeoJSON
st_write(NJcounties_fishnet, njgeojson_file)
```
...
Color Land Cover 2011 by MRLC Land Cover palette
```{r}
...
nlcdcols <- data.frame(coltab(lc_2011))
...
LCcolors
##        11        21        22        23        24        31 
## "#466B9F" "#DEC5C5" "#D99282" "#EB0000" "#AB0000" "#B3AC9F" 
##        41        42        43        52        71        81 
## "#68AB5F" "#1C5F2C" "#B5C58F" "#CCB879" "#DFDFC2" "#DCD939" 
##        82        90        95 
...
```{r, warning = FALSE, message = FALSE, results = "hide"}
NJHighways <-
  st_read("C:/Users/ferna/OneDrive/Documents/ArcGIS/Projects/CPLN 6750/HW5/njroads_MSA.geojson") %>%
  st_transform(st_crs(NJcounties)) %>%
  st_intersection(NJcounties)
```
...
highwayPoints_fishnet <- 
  aggregate(highwayPoints, NJcounties_fishnet, mean) %>%
...
ggplot() +
  ...
  labs(title = "Sensitive regions",
       subtitle = "Continous areas of either wetlands or forests/ngreater than 1 acre") +
  mapTheme
```
...
ggplot() +
  ...
  scale_fill_manual(values = palette5, name="Development/nDemand",
...
ggplot() +
  ...
  geom_point(data=fortBend_landUse, aes(x=xyC(fortBend_landUse)[,1], 
                                        ...
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(houstonHighways,filter(studyAreaCounties, NAME=="Fort Bend")), size=2) +
  scale_fill_manual(values = palette5, name="Population/nChange",
